...
<a name="FAQ1"></a>
**(1) Can Firewall completely protect my privacy?**

No - nothing can completely protect your privacy.
Firewall will do its best, but it is limited by the fact it must use the VPN service.
...
Android N will allow Firewall to be an [Always-On VPN](https://developer.android.com/preview/features/afw.html#always-on-vpn).
...
<a name="FAQ2"></a>
**(2)  Can I use another VPN application while using Firewall**

If the VPN application is using the [VPN service](http://developer.android.com/reference/android/net/VpnService.html),
then no, because Firewall needs to use this service. Android allows only one application at a time to use this service.

<a name="FAQ3"></a>
**(3) Can I use Firewall on any Android version?**
...
because Firewall uses the [Android VPN service](https://developer.android.com/reference/android/net/VpnService.html),
which was added in Android 4.0.

<a name="FAQ4"></a>
**(4) Will Firewall use extra battery power?**
...
<a name="FAQ6"></a>
**(6) Will Firewall send my internet traffic to an external (VPN) server?**
...
The [Android VPN service](http://developer.android.com/reference/android/net/VpnService.html) is being used to locally route all internet traffic to Firewall so no root is required to build a firewall application.
...
By showing all applications, Firewall allows you to control internet access even *before* such an update occurs.

<a name="FAQ8"></a>
**(8) What do I need to enable for the Google Play™ store app to work?**

You need 3 packages (applications) enabled (use search in Firewall to find them quickly):
...
Note that Firewall does not require any Google service to be installed.
...
If disabling Firewall is allowed to Tasker, any application can disabled Firewall too.
Allowing to disable a security application from other applications is not a good idea.

<a name="FAQ13"></a>
**(13) How can I remove the ongoing Firewall entry in the notification screen?**

* Long click the Firewall notification
* Tap the 'i' icon
...
this operation will disable any information or warning notifications from Firewall as well,
...
<a name="FAQ17"></a>
**(17) Why is Firewall using so much memory?**

It isn't, Firewall doesn't allocate any memory, except a little for displaying the user interface elements.
...
is incorrectly attributed to Firewall instead to the Google Play™ store app.

<a name="FAQ18"></a>
**(18) Why can't I find Firewall in the Google Play™ store app?**

Firewall requires at least Android 4.0, so it is not available in the Google Play™ store app for devices running older Android versions.
...
To be sure you can disable battery optimizations for Firewall manually like this:

```
Android settings > Battery > three dot menu > Battery optimizations > Dropdown > All apps > Firewall > Don't optimize > Done
```

This cannot be done from the application,
because according to Google Firewall is [not an application type allowed to do this](http://developer.android.com/training/monitoring-device-state/doze-standby.html#whitelisting-cases).

<a name="FAQ22"></a>
**(22) Can I tether / use Wi-Fi calling while using Firewall?**
...
which is shown by Android and not by Firewall, can unfortunately not be removed.
...
Unless you are just testing Firewall, there is no current reason to use them both, since they cover the same function (firewall),
...
* if not using filtering in Firewall, applications _need_ direct internet access (Wi-Fi and/or mobile) in AFWall+
* if using filtering, Firewall will _need_ internet access (Wi-Fi and/or mobile) in AFWall+
* if using filtering, when you un/reinstall Firewall, remember to RE-allow Firewall in AFWall+
...
<a name="FAQ32"></a>
**(32) Why is the battery/network usage of Firewall so high?**

This is because Android contributes battery and network usage which is normally contributed to other applications
to Firewall in IP filtering mode. The total battery usage is slightly higher when IP filtering mode is enabled.
...
Firewall "asks" Android to start the local VPN service,
but some Android versions contain a bug which prevents the VPN from starting (automatically).
Sometimes this is caused by updating Firewall.
Unfortunately this cannot be fixed from Firewall.
What you can try is to restart your device and/or revoke the VPN permissions from Firewall using the Android settings.
Sometimes it helps to uninstall and install Firewall again (be sure to export your settings first).
...
<a name="FAQ38"></a>
**(38) Why did Firewall stop running?**

On most devices, Firewall will keep running in the background with its foreground service.
On some devices (in particular some Samsung models), where there are lots of applications competing for memory, Android may still stop Firewall as a last resort.
Unfortunately this cannot be fixed from Firewall, and can be considered a shortcoming of the device and/or as a bug in Android.
You can workaround this problem by enabling the watchdog in the Firewall advanced options to check every 10-15 minutes.
...
Access to domain names shown in the application access log (drill down the Firewall application settings) will not be notified again,
...
<a name="FAQ45"></a>
**(45) Does Firewall handle incoming connections?**
...
If a purchased pro feature doesn't work [as described](https://www.Firewall.me/) while the free features of Firewall work properly
...
Developing Firewall was quite a challenge and [really a lot of work](https://www.openhub.net/p/Firewall/estimated_cost), but fun to do.
...
Firewall blocks traffic based on the IP addresses an application is trying to connect to.
...
This could result in Firewall too early or too late purging a DNS record from its own cache,
resulting in not recognizing an IP address or recognizing a wrong IP address.
You can try to workaround this by changing the DNS TTL value setting of Firewall.
This value is used as a minimum DNS TTL value in an attempt to mimick the behavior of Android.

<a name="FAQ49"></a>
**(49) Does Firewall encrypt my internet traffic / hide my IP address?**

Firewall is a firewall application that filters internet traffic on your device (see also [this question](#FAQ6)),
so it is not meant to and does not encrypt your internet traffic
and it is not meant to and does not hide your IP address.

<a name="FAQ50"></a>
**(50) Will Firewall automatically start on boot?**

Yes, Firewall will automatically be started on boot if you powered off your device with Firewall enabled.

<a name="FAQ51"></a>
**(51) Firewall blocks all internet traffic!**

Make sure you have put Firewall on the doze exception list (Android 6 Marshmallow or later)
and that Android allows Firewall to use the internet on the background.

Make sure you are not running Firewall in whitelist mode (check the Firewall default settings).

Some Android versions contain a bug resulting in all internet traffic being blocked.
Mostly you can workaround this bug by enabling filtering in the advanced Firewall settings.

<br />

**Firewall is supported for phones and tablets only, so not for other device types like on a television or in a car.**
